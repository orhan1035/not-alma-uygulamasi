...
<div class="max-w-2xl mx-auto p-6 bg-white/10 backdrop-blur-md rounded-3xl shadow-lg border border-white/20 mt-12 text-white">
    <h1 class="text-3xl font-extrabold mb-6 text-center drop-shadow-lg">Yeni Not Oluştur</h1>
    ...
    <form action="{{ route('notes.store') }}" method="POST" class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <a href="{{ route('notes.index') }}" class="text-white hover:underline">İptal</a>
            <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-semibold px-6 py-2 rounded-lg shadow transition">
                Kaydet
            </button>
        </div>
    </form>
